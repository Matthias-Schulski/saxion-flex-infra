...
sudo curl -LO "https://dl.k8s.io/release/$(curl -L -s https://dl.k8s.io/release/stable.txt)/bin/linux/amd64/kubectl" -o ~/MK/kubectl
sudo install -o root -g root -m 0755 ~/MK/kubectl /usr/local/bin/kubectl
...
sudo install -o root -g root -m 0755 ~/MK/minikube-linux-amd64 /usr/local/bin/minikube
