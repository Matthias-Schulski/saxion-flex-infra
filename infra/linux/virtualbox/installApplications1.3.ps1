...
write-host "Guestadditions wordt geïnstalleerd op $vmname. Wanneer gevraagd wordt om het wachtwoord, vul het wachtwoord $password in."
ssh -p 2222 -t ubuntu@127.0.0.1 'sudo apt install -y bzip2 tar; sudo mount /dev/cdrom /mnt; cd /mnt; sudo sh ./VBoxLinuxAdditions.run'

    #DIRECTORY AANMAKEN VOOR SCRIPTS IN VM
    write-host "Directory aanmaken in VM (scripts)" -ForegroundColor Yellow
    VboxManage guestcontrol $($VM.VMName) mkdir "/home/$hostname/scripts" --username ubuntu --password ubuntu
    ...
    foreach ($app in $vm.VMApplications) {
